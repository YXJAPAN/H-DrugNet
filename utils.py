...
class EarlyStopping:
    """Early stops the training if validation score does not improve after a given patience."""
    def __init__(self, patience=20, verbose=True, delta=0, path='checkpoint.pt', mode='min', trace_func=print):
        """
        Args:
    ...
    def save_checkpoint(self, score, model):
        ...
        if self.verbose:
            if self.mode == 'min':
                self.trace_func(f'Validation score decreased ({self.best_score:.6f} --> {score:.6f}).  Saving model ...')
            else: # mode == 'max'
