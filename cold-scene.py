...
from new_Net import * # 修改点: 从 new_Net.py 导入模型
...
# 修改点: 适配 A3Net_DiffPool 的训练函数
def train_diffpool(model, device, train_loader, optimizer, lamb, epoch, log_interval, sideEffectsGraph, id, DF, not_FC, eps,
                   lambda_link, lambda_entropy):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        # 接收模型返回的辅助损失
        out, x, x_e, link_loss, entropy_loss = model(data, sideEffectsGraph, DF, not_FC)
        
        raw_label = data.y
        pred = out.to(device)
        
        # 计算总损失
        main_loss = loss_fun(pred.flatten(), raw_label.flatten(), lamb, eps)
        total_loss = main_loss + lambda_link * link_loss + lambda_entropy * entropy_loss
        
        total_loss.backward()
        optimizer.step()
        avg_loss_list.append(total_loss.item())

        if (batch_idx + 1) % log_interval == 0:
            print(
                '{} Train epoch: {} [{}/{} ({:.0f}%)]\tTotal Loss: {:.6f} (Main: {:.6f}, Link: {:.6f}, Entropy: {:.6f})'.format(
                    id, epoch, (batch_idx + 1) * len(data.y), len(train_loader.dataset),
                           100. * (batch_idx + 1) / len(train_loader), total_loss.item(),
                    main_loss.item(), link_loss.item(), entropy_loss.item()))
                    
    return sum(avg_loss_list) / len(avg_loss_list)
...
def predict(model, device, loader, sideEffectsGraph, DF, not_FC):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(loader):
            ...
            data = data.to(device)
            # 修改点: 适配模型输出
            out, _, _, _, _ = model(data, sideEffectsGraph, DF, not_FC)
...
def evaluate(model, device, loader, sideEffectsGraph, DF, not_FC, result_folder, id):
    ...
    with torch.no_grad():
        ...
        for data in loader:
            ...
            data = data.to(device)
            # 修改点: 适配模型输出
            output, _, _, _, _ = model(data, sideEffectsGraph, DF, not_FC)
            pred = output.cpu()
...
def main(modeling, metric, train_batch, lr, num_epoch, knn, weight_decay, lamb, log_interval, cuda_name, frequencyMat,
         id, mask, result_folder, save_model, DF, not_FC, output_dim, eps, pca, num_clusters, lambda_link, lambda_entropy): # 修改点: 接收新参数
    print('\n=======================================================================================')
    print('\n第 {} 次训练：\n'.format(id))
    ...
    print('Device: ', device)
    # 修改点: 实例化新模型
    model = modeling(input_dim=input_dim, output_dim=output_dim, num_drug_clusters=num_clusters).to(device)
    ...
    for epoch in range(num_epoch):
        # 修改点: 调用新的训练函数
        train_loss = train_diffpool(model=model, device=device, train_loader=train_loader, optimizer=optimizer, lamb=lamb,
                           epoch=epoch + 1, log_interval=log_interval, sideEffectsGraph=sideEffectsGraph,
                           id=id, DF=DF, not_FC=not_FC, eps=eps, lambda_link=lambda_link, lambda_entropy=lambda_entropy)
        train_losses.append(train_loss)

    torch.save(model, "model_cold_diffpool.pt")
...
if __name__ == '__main__':
    ...
    # 修改点: 添加新参数
    parser.add_argument('--lambda_link', type=float, required=False, default=1.0,
                        help='Weight for link prediction loss')
    parser.add_argument('--lambda_entropy', type=float, required=False, default=0.1, help='Weight for entropy loss')
    parser.add_argument('--num_clusters', type=int, required=False, default=32,
                        help='Number of drug clusters for DiffPool')
    ...
    modeling = [A3Net_DiffPool][args.model] # 修改点: 默认模型改为 A3Net_DiffPool
    ...
    for (id, frequencyMat, mask) in split_data(tenfold):
        ...
        main(modeling, metric, train_batch, lr, num_epoch, knn, weight_decay, lamb, log_interval, cuda_name,
             ...
             num_clusters, lambda_link, lambda_entropy)
