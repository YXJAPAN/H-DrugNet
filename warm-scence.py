...
# 在 warm-scence-modified.py 中替换旧的 train 函数
def train_diffpool(model, device, train_loader, optimizer, lamb, epoch, log_interval, sideEffectsGraph, DF, not_FC, eps,
                   lambda_link, lambda_entropy):
    print(f'Training Epoch {epoch} with DiffPool...')
    model.train()
    avg_loss_list = []
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        main_loss = loss_fun(pred.flatten(), train_label.flatten(), lamb, eps)
        ...
        if (batch_idx + 1) % log_interval == 0:
            print(
                'Train epoch: {} [{}/{} ({:.0f}%)]\tTotal Loss: {:.6f} (Main: {:.6f}, Link: {:.6f}, Entropy: {:.6f})'.format(
                    epoch, (batch_idx + 1) * len(data.y), len(train_loader.dataset),
...
def predict(model, device, loader, sideEffectsGraph, raw, DF, not_FC):
    ...
    with torch.no_grad():
        ...
        for data in loader:
            ...
            data = data.to(device)
            output, _, _, _, _ = model(data, sideEffectsGraph, DF, not_FC) # 忽略辅助损失
...
def main(modeling, metric, train_batch, lr, num_epoch, knn, weight_decay, lamb, log_interval, cuda_name, frequencyMat,
         id, mask, result_folder, save_model, DF, not_FC, output_dim, eps, pca, num_clusters,lambda_link, lambda_entropy):
    print('\n=======================================================================================')
    print('\n第 {} 次训练：\n'.format(id))
    ...
    for epoch in range(num_epoch):
        train_loss = train_diffpool(model=model, device=device, train_loader=train_loader, optimizer=optimizer,
                                    lamb=lamb,
                                    epoch=epoch + 1, log_interval=log_interval, sideEffectsGraph=sideEffectsGraph,
                                    DF=DF, not_FC=not_FC, eps=eps,
                                    lambda_link=lambda_link, lambda_entropy=lambda_entropy)
...
if __name__ == '__main__':
    ...
    modeling = [A3Net_DiffPool][args.model]
    ...
    for (id, frequencyMat, mask) in split_data(tenfold):
        ...
        main(modeling, metric, train_batch, lr, num_epoch, knn, weight_decay, lamb, log_interval, cuda_name,
             frequencyMat, id + 1, mask, result_folder, save_model, DF, not_FC, output_dim, eps, pca, num_clusters,lambda_link, lambda_entropy)
